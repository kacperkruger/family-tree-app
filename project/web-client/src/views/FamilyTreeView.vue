...
const selectPerson = (personId: string | number | undefined): void => {
  selectedPerson.value = familyTreeStore.getPersonDetailsFromTree(personId)
}
...
<template>
  <div class="view">
    ...
    <SideMenuComponent v-if="selectedPerson && !readOnly" :menu-status="isOpenEditPerson" @closeMenu="isOpenEditPerson = false">
      <div class="flex flex-col gap-2 h-1/2 justify-end">
    ...
    <SideMenuComponent v-if="selectedPerson" :menu-status="isOpenPersonDetails"
      ...
      <div class="flex flex-col gap-2 h-1/2 justify-end text-lg">
        ...
        <div v-if="!readOnly" class="flex gap-4 w-full justify-end">
          <button @click="openEditPersonMenu"
                  class="p-2 border-green-500 text-white rounded text-sm bg-green-500 hover:bg-green-600">Edit
          </button>
          <button @click="deletePerson"
