...
export const useUsersStore = defineStore('users', () => {
    ...
    const getUsersByPersonsDetails = async (surnames: string[], dateOfBirth?: string) => {
        if (!surnames.length) {
            await getAllUsers()
            return
        }
        try {
            const response = await axios.get(`${import.meta.env.VITE_API_HOST_URL}/api/v1/family-trees/users?${surnames.map(surname => `surname=${surname}`).join('&')}${dateOfBirth ? `&dateOfBirth=${dateOfBirth}` : ''}`, {withCredentials: true})
            users.value = response.data.users
        } catch (e) {
            if (isAxiosError(e) && e.response?.status === 401) {
                authStore.logout()
    ...
    return { users, getAllUsers, getUser, getUserByUsername, getUsersByPersonsSurnames: getUsersByPersonsDetails }
})